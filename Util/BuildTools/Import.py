...
def invoke_commandlet(name, arguments):
    ...
    uproject_path = os.path.join(CARLA_ROOT_PATH, "Unreal", "CarlaUE5", "CarlaUE5.uproject")
    run = "-run=%s" % (name)
...
def generate_package_file(package_name, props, maps):
    ...
    package_config_path = os.path.join(CARLA_ROOT_PATH, "Unreal", "CarlaUE5", "Content", package_name, "Config")
...
def build_binary_for_navigation(package_name, dirname, maps):
    ...
    for umap in maps:
        ...
        for tile in tiles:
            ...
            nav_folder_target = os.path.join(CARLA_ROOT_PATH, "Unreal", "CarlaUE5", "Content", package_name, "Maps", target_name, "Nav")
